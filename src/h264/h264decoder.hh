...
public:
  ...
  enum ErrorResilience : int {
    SkipFrame = 0,   // skip error frames
    ErrorConcealment = 1,      // usr error concealment in ffmpeg
  };

  H264Decoder(const uint16_t display_width,
  ...
  const H264Decoder & operator=(const H264Decoder & other) = delete;
  H264Decoder(H264Decoder && other) = delete;
  H264Decoder & operator=(H264Decoder && other) = delete;
