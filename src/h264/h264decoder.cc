...
H264Decoder::H264Decoder(const uint16_t display_width,
                 const uint16_t display_height,
                 const int lazy_level,
                 const int er,
                 const string & output_path)
  : display_width_(display_width), display_height_(display_height),
    lazy_level_(), er_(), output_fd_(), decoder_epoch_(steady_clock::now()),
    codec_ctx_(nullptr),codec_(nullptr),frame_(nullptr),pkt_(nullptr)
{
  // validate lazy level
  ...
  // validate er
  if (er < DECODE_DISPLAY or er > NO_DECODE_DISPLAY) {
    throw runtime_error("Invalid er: " + to_string(er));
  }
  er_ = static_cast<ErrorResilience>(er);
  ...
  if (lazy_level <= DECODE_ONLY) {
    worker_ = thread(&H264Decoder::worker_main, this);
    cerr << "Spawned a new thread for decoding and displaying frames" << endl;
  }
}

bool H264Decoder::add_datagram_common(const Datagram & datagram)
{
...
void H264Decoder::add_datagram(const Datagram & datagram)
{
...
void H264Decoder::add_datagram(Datagram && datagram)
{
  if (not add_datagram_common(datagram)) {
    return;
  }
...
bool H264Decoder::next_frame_complete()
{
  {
...
void H264Decoder::consume_next_frame()
{
  Frame & frame = frame_buf_.at(next_frame_);

  if (er_ == SkipFrame && not frame.complete()) {
...
void H264Decoder::advance_next_frame(const unsigned int n)
{
  next_frame_ += n;
...
void H264Decoder::clean_up_to(const uint32_t frontier)
{
...
double H264Decoder::decode_frame(const Frame & frame)
{
  if (er_ == SkipFrame && not frame.complete()) {
  ...
  // 读取数据包
  ret = av_packet_from_data(pkt_, decode_buf.data(), frame_size);
  if (ret < 0) {
...
void H264Decoder::display_decoded_frame(VideoDisplay & display)
{
  ...
  while ((ret = avcodec_receive_frame(codec_ctx_, frame_))==0) {
      ...
      display.show_frame(frame_);
      
  }
}

void H264Decoder::worker_main()
{
  ...
  // initialize a H264 decoding context
  ...
  codec_ctx_->thread_count = max_threads;
