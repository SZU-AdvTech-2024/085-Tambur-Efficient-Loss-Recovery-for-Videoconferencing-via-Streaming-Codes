    ...
    frame_rate_(frame_rate), output_fd_()
{
  ...
  codec_ctx->time_base = (AVRational){1, frame_rate_};
  codec_ctx->framerate = (AVRational){frame_rate_, 1}; // WebRTC uses a 90 kHz clock
  ...
  codec_ctx->thread_count = 4; // encoder threads; should equal to column tiles below
  //cfg_.rc_resize_allowed = 0; // WebRTC enables spatial sampling
  codec_ctx->rc_buffer_size = 1000; 
  codec_ctx->rc_initial_buffer_occupancy = 500; 
  codec_ctx->rc_buffer_size = 1000;
...
H264Encoder::~H264Encoder()
{
  if (vpx_codec_destroy(&context_) != VPX_CODEC_OK) {
    cerr << "~H264Encoder(): failed to destroy H264 encoder context" << endl;
  }
}
